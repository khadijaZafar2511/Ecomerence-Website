      ...
      {data.products.map((p) =>
        p.id != id ? null : (
          <div
            id="main"
            key={p.id}
            className="flex lg:flex-row md:flex-row sm:flex-row flex-col lg:gap-3 items-center    w-full h-screen  "
          >
            <div1 className="lg:h-full lg:w-1/2 md:h-full md:w-1/2  sm:h-full sm:w-1/2 h-4/7 w-full flex  justify-center lg:mt-35 md:mt-30 sm:mt-30 mt-5">
              ...
              <div className="hidden lg:block md:block sm:block lg:h-9/10 lg:w-7/10 md:h-1/2 md:w-9/10 sm:h-4/5 sm:w-9/10 h-full w-full  bg-radial from-[#c7c1B4] via-[#C4BEB0] to-[#9F9888] lg:ml-4 ">
                <img
                  src={p.images[0] ? p.images[0] : p.images[1]}
                  className="h-full  w-full "
                />
                ...
                <button
                  onClick={() => {
                    (dispatch({ type: "setcount" }),
                      dispatch({ type: "setcart", payload: p, qnty1: qnty }));
                  }}
                  className=" border rounded-4xl h-16 md:h-13 lg:h-20 lg:w-1/2 w-4/5 text-[13px] text-white bg-gray-800 mt-10 lg:ml-20  ml-9"
                >
                  Add To Cart
                </button>

                <div className="  lg:w-3/4 lg:h-4/5 md:w-3/4 md:h-1/3 sm:w-4/5 sm:h-1/3 w-full h-2/3 lg:ml-3 md:ml-3 sm:ml-3 px-2 border-t border-t-gray-400 mt-7 text-sm  flex flex-col gap-3 overflow-y-scroll  [&::-webkit-scrollbar]:w-1    [&::-webkit-scrollbar-track]:bg-gray-100  [&::-webkit-scrollbar-thumb]:bg-gray-500  [&::-webkit-scrollbar-thumb]:rounded-none ">
